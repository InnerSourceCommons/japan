...
アジャイルやスクラムの工夫は、オフィスで仕事をする仲間を開発者だけにせず、ソフトウェアのユーザーやその他のステークホルダーたちとも定期的でカジュアルな情報共有をすることです。
この工夫により、要求仕様書や設計書といったドキュメントの必要性を減らすことができます。
...
実際、文献においてこの2つを組み合わせたチームが報告されています <sup>[1]</sup>。
...
例えば、_トラステッドコミッター_ <sup>[2]</sup> という憧れの称号に相応しい開発者を見つけるには、受け入れられたコミットの数で判断することが通例です。
トラステッドコミッターは、他の開発者の成果をレビューして採否を判定する役割を持ちます。
また、自身はそうしたレビュープロセスを経ることなく変更する権限を持ちます。
さらに、品質基準を満たさないコードを提出した伸び盛りの開発者を育てる役割も担います。

- [2]: (訳注) OSS において、コミッターは開発方針やコード変更の最終判断を担う中心的存在です。インナーソースでは、ある組織内のソフトウェアに対して一定以上のコントリビューションをしたと信頼された人は、その組織内の所属部署に関わらずトラステッドコミッターと呼ばれます。お墨付きコミッターと言ってもよいかもしれません。

### 明文化の重要性

さらなる OSS の信念は徹底的な明文化です <sup>[3]</sup>。
...
メーリングリストは議論による意思決定に欠かせないツールとなっているでしょう <sup>[4]</sup>。
...
- [3]: (訳注) 原文での節名は The Importance of Documentation です。ここでの documentation とはドキュメントにすることだけでなく、チャットやチケットのコメントも含むため、明文化と表現しました。
- [4]: (訳注) 2022 年現在では Gitter や Slack といったチャットツールがメーリングリストに代わって利用される場合も増えているでしょう。
...
アジャイルの専門家である Mike Cohn は、2009年にテストピラミッド <sup>[5]</sup> という概念を提唱しました。
...
また、そのような企業は[クローズドコア](http://radar.oreilly.com/2011/12/could-closed-core-prove-a-more.html)と呼ばれることがあります <sup>[8]</sup>。
インフラ部分を OSS にすることにより、プログラミングに関わる数多の人々が利益を受け、社員の採用と教育が効率的になり、企業は外からのコントリビューションを受け取ることができるようになるのです。

- [6]: (訳注) プロプライエタリ(proprietary)とは、"独占的"、"専売" を意味する形容詞です。プロプライエタリソフトウェアとは、1社が独占的に提供し、ソースコードやその他の中間成果物が非公開であるソフトウェアを主に意味します。本文のように、OSS の対義語として使われる場合がほとんどです。
- [7]: (訳注) SaaS に限らず、デスクトップアプリや組込みソフトウェアでも同様の選択をすることがあるでしょう。
- [8]: (訳注) オープン・クローズ戦略の方が日本の企業人にとって馴染みがあるのではないでしょうか。ただし、オープン・クローズ戦略はソフトウェアの開発や提供の方法ではなく知財マネジメントの文脈で語られることに注意が必要です。興味のある読者は[経済産業省発行の2013年版ものづくり白書](https://www.meti.go.jp/report/whitepaper/mono/2013)第3節 その3を参照するとよいでしょう。

## PayPal がインナーソースを採用するまで

PayPal がインナーソースを採用するまでの過程には企業としての意志決定の積み重ねがあり、歴史的で大規模なものです。
インナーソース の採用は、それまでに何度かあった企業変革と同様に、ツールと企業文化の戦略的変更として進められました。

PayPal がインナーソースを採用する以前に行われた変革とその結果は次の通りです。
...
つまり、PayPal ではインナーソースを採用する前からオープンソースにゆかりのある技術を部分的に採用していたということです。
...
PayPal のインナーソースは、米国外の様々な支社で働くセールスエンジニアから始まりました <sup>[9]</sup>。
...
これと同時にドキュメントが不十分であるという問題もあり、他のチームの開発者との間で飛び交う緊急の Eメールのやり取りでカバーしていました。
これらのメールには重要な情報が含まれていましたが、その他の多くの開発者が目にすることはありませんでした。
多くの支社では、そうしたメッセージをそれぞれ独自の Wiki にカット＆ペーストする試みを始める始末でした。

コアチームのコードに対するドキュメントが不十分であるために各地域のエンジニアがコードを十分に理解できず、コアチームが受け入れやすい変更に仕上げられなかったのです <sup>[10]</sup>。

そこで、各支社のエンジニアはサンノゼの本社に飛び、1ヶ月間コアチームから直接教えを受けました。
しかし、多くのトラステッドコミッターを育てられるほど知識を十分に広められたとはいえず、投資に見合うものではありませんでした。
ドキュメントの拡充も全く進みませんでした。
エンジニアたちが母国に帰国した後は、文面による問い合わせを通じて、メンターとなってくれるコアチームの開発者を探すことを余儀なくされました。

このように多大な労力を払った結果、インナーソースのほうが良い方法かもしれないと考えられ、採用されることになりました。
そして、各チームはインナーソースによってプロジェクトが本当に効率化することに気づきました。
コアチームのトラステッドコミッターも、そのプロセスがはるかに優れていることに気づきました。
離れた地域にいるエンジニアであっても GitHub からコードをプルできるようになりました。
...
- [9]: (訳注) [PayPal には米国のほかに様々な拠点があり](https://careers.pypl.com/locations/default.aspx)、[さらに広い国々を市場としています](https://www.paypal.com/en/webapps/mpp/country-worldwide)。
- [10]: (訳注) これは原著にはない文ですが、PayPal の背景を理解するためには必要と判断して追加しました。

### よりスピーディな開発プロセス

PayPal の黎明期は比較的モノリシックな C++ プラットフォームでスタートしました。
その後、レガシーな要素は C++ が残りながらも、新規開発については Java の Spring を使って、よりモジュール化された方法で行われるようになりました。
その移行後も、開発者の Jeff Harrell が言うには、各製品のコードには「部族内知識」という社内の一部でしか通用しない知識が多く埋め込まれていました。
そのため、小さな変更を取り込むにも数週間が必要で、新入社員には6週間のトレーニング期間が必要だったのです。
...
多くの企業は、OSS という技術が純粋に高品質で無償であることを魅力とみなしており、それをきっかけにした OSS の採用はコミュニティへの継続的な参入につながると考えています。

これが PayPal が Node.js を採用したときに起こったことです。
いまでこそ「JavaScript プログラマーは Node.js に代わる技術を探している」と指摘するコメンテーターもいるほどピークを過ぎて成熟した Node.js ですが、PayPal が Node.js コミュニティに参入した時期はまだ初期でした。
そのため十分にコントリビューションする余地があったのです(本節の後半で説明するように、OSS の変化する速度が採用する技術を左右します)。
...
Kraken の開発理由は、社内のあらゆるコードに見られる「部族内知識」という社内の一部でしか通用しない知識を、一般的に覚える価値のある知識に昇華させることでした。
そのため、Harrell のチームは Kraken を OSS として開発することにしたそうです。
こうして、Kraken はおそらく彼らの最も成功した OSS プロジェクトになりました。

Venkatakrishnan は、この時 PayPal に導入した OSS 由来のプラクティスを次のように挙げています。

* 高い品質基準を設定すること。コードをコミットする前にコードカバレッジテストが実行され、カバレッジが少なくとも 90% 以上でなければならない。コードのプッシュに対して自動ビルドが実行され、そのコードがマージされるのに十分な品質があるか確認される必要がある。
...
* <!-- textlint-disable -->すべての専門知識を単一のチームから得る必要はないとエンジニアに教えること。世界中のどこからでも支援を受けることができるのが OSS である。OSS プロジェクトにコントリビュートすることで会社の知名度が上がり、積極的に取り組む姿勢をコミュニティに示すことができる。<!-- textlint-enable -->
...
投稿者とトラステッドコミッターの間で交わされるコメントのやりとりは、投稿者にとっても他のプログラマーにとってもリアルタイムに学習できる良い教材となります。

### 文化の変化

インナーソースの採用という企業変革には、従業員の不安と期待への慎重な対応が求められます。
PayPal はこの変革を支援するために、外部の OSS 開発専門家を雇用しました。

多くの企業の取締役やベテランは <sup>[11]</sup>、「最も重要な文化的変化とは、従業員が他チームにコードを提供する事への自信を与えることだ」と考えるに至る経験をしてきたようです。
そのためには、徹底的なドキュメンテーションと丁寧な個人指導が欠かせません。
特に「バグに苦情を言う社員の習慣を修正する習慣へ変えなければならない」と Edelman は指摘しています。
...
別の企業の調査では、新しいコントリビューションは自分のメンテナンスの手間を増やすことになると、プログラマーが心配していました <sup>[12]</sup>。
...
コードを一般公開用のリポジトリに移動し、社内でやっていたバグ報告やコード編集といったコントリビューションを社外でも同様の方法でやるだけなのです。
...
- [11]: (訳注) 原文では `Observers from many company` と書かれています。この観測者(observers)とは、取締役会に参加するものの議決権のないボードオブザーバーと呼ばれる人か、企業の変遷を観測してきた通常の取締役かベテラン社員を意味すると考え、このように訳しました。また、原文には明示されていませんが、インナーソースによる文化的変化を経験した人々を指しているでしょう。
- [12]: Stol, KJ, AB Muhammad, et al. [“A comparative study of challenges in integrating Open Source Software and Inner Source Software.”](http://www.cs.rug.nl/paris/papers/IST11.pdf) _Information and Software Technology_ 53 (2011): 1319–1336\.
